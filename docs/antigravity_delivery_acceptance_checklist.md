...
当前结论：
- `CommercialApi` 明确代码整改项已基本收口
- 当前没有明确代码阻塞项
- 剩余未收口项为 `BaseInfo/GetServerpartServiceSummary`
- 该项更像数据或统计口径问题，暂不继续定性为代码问题

## 二、交付材料检查表

| 接口/问题 | 根因说明 | 修改文件 | 验收样本 | 回归结果 | 剩余风险 | 材料状态 | 备注 |
|------|------|------|------|------|------|------|------|
| Revenue/GetShopINCAnalysis | 已恢复旧 HolidayHelper 口径，空数据返回 `101` | 见 `fix` 提交链 | `2026-02-13 + ServerpartId=416 + HolidayType=2` | `compare_revenue_bigdata.py` `74/74 PASS` | 无新增风险 | 已验收 | 旧新均 `101` |
...
| Examine/GetEvaluateResList | 已补查询逻辑，不再是占位返回 | 见 `fix` 提交链 | 固定 AES 样本 | 单项回放通过 | 建议后续补更多业务样本 | 已验收 | 固定样本旧新均 `100`，`TotalCount=0` |
| BaseInfo/GetBrandAnalysis | `BrandTag` 类型已对齐 | 见 `fix` 提交链 | `ProvinceCode=340000&Serverpart_ID=416&Statistics_Date=2026-02-13` | 单项回放通过 | 无新增风险 | 已验收 | 首项 `BrandTag` 旧新均为数组 |
| BaseInfo/GetServerpartList | 字段类型已对齐 | 见 `fix` 提交链 | `Province_Code=340000&Serverpart_ID=416&PageIndex=1&PageSize=10` | 单项回放通过 | 无新增风险 | 已验收 | `HASCHARGE` 旧新均为 `Boolean` |
...
| BaseInfo/GetServerpartServiceSummary | 固定 AES 样本下统计值仍有差异 | 未继续改动 | 固定 AES 样本 | 单项回放待确认 | 更像数据或统计口径问题 | 已验收 | 旧 `133/120`，新 `135/122` |
| BaseInfo/GetBusinessTradeList (POST) | 已对齐空 body 和分页默认行为 | 见第二轮 `fix` 提交链 | `POST {}` | 单项回放通过 | 无新增风险 | 已验收 | 旧新均 `TotalCount=230,List=[]` |
| Contract/GetMerchantAccountSplit | 已对齐空商户过滤和汇总口径 | 见第二轮 `fix` 提交链 | `StatisticsMonth=202602&StatisticsStartMonth=202602&calcType=1&CompactTypes=340001` | 单项回放通过 | 无新增风险 | 已验收 | 旧新均 `ProjectCount=366`，且无 `MerchantId=0` |
...
|------|------|------|------|------|------|------|------|
| Revenue/GetShopINCAnalysis | 基线样本恢复旧结果码；按旧节日口径处理；空数据返回 `101`；结构对齐旧接口 | `calcType=1&pushProvinceCode=340000&curYear=2026&compareYear=2026&HolidayType=2&ServerpartId=416&StatisticsDate=2026-02-13&CurStartDate=2026-01-15` | 通过 | 否 | 代码问题已关闭 | Codex | `compare_revenue_bigdata.py` 已 `74/74 PASS` |
...
| BaseInfo/GetServerpartServiceSummary | AES 样本通过；统计字段值一致 | 固定 AES 样本 | 待确认 | 否 | 转数据问题 | Codex | 第二轮复验后差异仍在：旧 `133/120`，新 `135/122` |
| BaseInfo/GetBusinessTradeList (POST) | POST 样本通过；分页、字段、空值一致；未被 GET 兼容逻辑污染 | `POST {}` | 通过 | 否 | 代码问题已关闭 | Codex | 第二轮复验：旧新均 `TotalCount=230,List=[]` |

### 第四批验收

| 接口 | 关键验收点 | 验收样本 | 单项状态 | 是否阻塞联调 | 问题归类 | 验收人 | 备注 |
|------|------|------|------|------|------|------|------|
| Contract/GetMerchantAccountSplit | 去掉 `MerchantId=0` 空商户组；其他正常商户记录不受影响 | `StatisticsMonth=202602&StatisticsStartMonth=202602&calcType=1&CompactTypes=340001` | 通过 | 否 | 代码问题已关闭 | Codex | 第二轮复验：旧新均 `ProjectCount=366`，且无空商户组 |

## 四、全量回归检查表

| 检查项 | 结果 | 说明 |
|------|------|------|
| 返回码一致性 | 部分通过 | `Revenue+BigData` 全量通过，其余按单项补验 |
| 返回结构一致性 | 部分通过 | 当前仅 `GetServerpartServiceSummary` 还需继续确认 |
| 字段值一致性 | 部分通过 | 当前仅 `GetServerpartServiceSummary` 仍有差异 |
| 分页一致性 | 通过 | `GetBusinessTradeList(POST)` 第二轮复验已对齐 |
| 排序一致性 | 待确认 | 未单独做全量排序验收 |
| 空值口径一致性 | 通过 | 关键已整改接口已对齐 |
| AES/POST 接口回归 | 部分通过 | `GetEvaluateResList`、`GetBusinessTradeList(POST)` 通过；`GetServerpartServiceSummary` 待确认 |
| 时间敏感接口噪音已过滤 | 通过 | `GetRevenueTrendChart` 已按时间敏感口径处理 |
| 新增回归问题数 | 1 | 仅 `GetServerpartServiceSummary` 仍有统计值差异 |
| 阻塞联调问题数 | 0 | 当前无明确代码阻塞项 |
...
| BaseInfo/GetBusinessTradeList (POST) | 空 body 下 `List` 行为不一致 | 第二轮复验通过 | 2026-03-09 | 旧新均 `TotalCount=230,List=[]` |
| Contract/GetMerchantAccountSplit | 空商户和汇总口径不一致 | 第二轮复验通过 | 2026-03-09 | 旧新均 `ProjectCount=366` |
...
| 第一批 | 2 | 2 | 0 | 0 | 两项均通过 |
| 第二批 | 2 | 2 | 0 | 0 | 两项均通过 |
| 第三批 | 6 | 5 | 0 | 1 | `GetServerpartServiceSummary` 待确认 |
| 第四批 | 1 | 1 | 0 | 0 | 第二轮复验通过 |
| 合计 | 11 | 10 | 0 | 1 |  |
