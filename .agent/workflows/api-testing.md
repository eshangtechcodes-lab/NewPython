...
- 新 Python API 服务已启动：`http://127.0.0.1:8080/CommercialApi`
  - 启动方式：在 `D:\AISpace\EShangPython` 目录下执行 `python main.py`
...
| 脚本 | 用途 | 频率 |
|------|------|------|
| `scripts/baseline/baseline_collect.py` | 并发采集旧 API 全部响应缓存到 JSON | 一次性（数据变化时重采） |
| `scripts/compare/compare_cached.py` | 读缓存 + 调新 API → 快速对比 | 每次迭代后运行 |
| `scripts/compare/compare_all.py` | 串行实时对比新旧 API（慢，备用） | 全量验证时用 |
| `scripts/_archived/perf_check_old_api.py` | 旧 API 性能基准测试 | 仅分析性能时用 |
...
```powershell
python scripts/baseline/baseline_collect.py
```
...
```powershell
python scripts/compare/compare_cached.py
```
