...
| Python Service 目录 | `D:\AISpace\EShangPython\services\` |
| Python Router 目录 | `D:\AISpace\EShangPython\routers\eshang_api_main\` |
| 全量 Manifest | `D:\AISpace\EShangPython\scripts\manifests\endpoint_case_library.json` |
| 对比脚本 | `D:\AISpace\EShangPython\scripts\compare_api.py` |
| 通用响应模型 | `D:\AISpace\EShangPython\models\base.py` |
| 数据库 Helper | `D:\AISpace\EShangPython\core\database.py` |
| 全量对比结果 | `D:\AISpace\EShangPython\docs\session4_full.json` |
| 模块修改计划 | `D:\AISpace\EShangPython\docs\session4_implementation_plan.md` |
...
```
find_by_name "*模块名*" D:\AISpace\EShangPython\services\
find_by_name "*模块名*" D:\AISpace\EShangPython\routers\
```
...
os.system(f'python scripts/compare/compare_api.py --manifest {tmp_path} --report {report_path}')
```

**示例：Budget 模块的验证脚本** `scripts/runners/run_budget_compare.py`：
...
执行命令：
```powershell
python scripts/runners/run_budget_compare.py
```
...
Start-Process -FilePath python -ArgumentList "-m","uvicorn","main:app","--host","0.0.0.0","--port","8080","--workers","1" -WorkingDirectory "D:\AISpace\EShangPython" -WindowStyle Hidden
