...
"""
补充全量同步 - 第2轮
修复：T_SHOPCOUNT 表结构不匹配需要重建
加：T_BUDGETDETAIL_AH, T_ENDACCOUNT_TEMP, T_BUDGETPROJECT_AH, T_BUDGETEXPENSE
"""
import oracledb
import dmPython
import time
...
TYPE_MAP = {
    ...
    "FLOAT": "DOUBLE", "CLOB": "CLOB", "NCLOB": "CLOB", "BLOB": "BLOB",
    "RAW": "VARBINARY", "LONG": "CLOB",
}
...
# 需要处理的表（表名, schema, 是否需要重建）
TABLES = [
    ("T_SHOPCOUNT",        "HIGHWAY_STORAGE",  True),   # 需要 drop + 重建
    ("T_BUDGETDETAIL_AH",  "FINANCE_STORAGE",  False),  # 清空+全量
    ("T_ENDACCOUNT_TEMP",  "HIGHWAY_SELLDATA", False),  # 清空+全量
    ("T_BUDGETPROJECT_AH", "FINANCE_STORAGE",  False),  # 清空+全量
    ("T_BUDGETEXPENSE",    "HIGHWAY_STORAGE",  False),  # 清空+全量
]
...
def main():
    print("=" * 70)
    print("Oracle → Dameng 全量数据补充同步 (第2轮)")
    print("=" * 70)
    results = []
